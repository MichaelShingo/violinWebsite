...
const BlobMenu: FC = () => {
	const isMenuOpen: boolean = useAppSelector(
		(state) => state.locationReducer.value.isMenuOpen
	);
	const isMenuHovered: boolean = useAppSelector(
		(state) => state.locationReducer.value.isMenuHovered
	);
	const hoveredMenuInfo: string = useAppSelector(
		(state) => state.locationReducer.value.hoveredMenuInfo
	);
	const pathname = usePathname();
	const dispatch = useDispatch();

	const rightAlignedPaths = [
		...menuItems.filter((item) => item.menuPosition === 'right').map((item) => item.link),
		urls.proposals,
		urls.hireMusicians,
	];
	const isRightLink = rightAlignedPaths.includes(pathname);
	...
	const calcMenuPosition = () => {
		...
		if (isMenuOpen) {
			...
			if (isRightLink) {
				res =
					'md:translate-x-[40vw] sm:translate-x-[35vw] translate-x-[30vw] lg:translate-x-[40vw] xl:translate-x-[41vw] 2xl:translate-x-[43vw]';
			}
		}
		return res;
	};
	...
	return (
		<div
			className={twJoin([
				'fixed z-[100] h-[200vh] w-dvw flex justify-center',
		...
		>
			<div
				...
				className={twJoin([
					...
					isMenuOpen
						? 'bg-black/75 backdrop-blur-md pointer-events-auto'
			...
			>
				...
				<div
					className={twJoin([
						'absolute -z-10 w-24 h-24 transition duration-700 pointer-events-auto',
						isMenuOpen ? '-rotate-[90deg]' : 'rotate-45',
					])}
				>
				...
				<div
					className={twJoin([
						isMenuOpen ? 'scale-[55%]' : isRightLink ? 'scale-[43%]' : 'scale-[48%]',
